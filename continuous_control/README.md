...
The environment is considered solved, when the average (over 100 episodes) of the score (across all the agents) is at
least +30.
...
The notebook `Continuous_Control.ipynb` contains all the steps required to train the agent and run it inside the env.

The path to the Reacher binaries must be specified in step 2.

### Docker

Alternatively, the `Dockerfile` provided can be built and ran (`docker build -t continous_control .` followed
by `docker run -d continuous_control -p 8888:8888`). It will create the environment for you and start a Jupyter notebook
ready to train and run the agent with this environment.
